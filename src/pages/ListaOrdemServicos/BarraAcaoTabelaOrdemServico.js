    ...
    return (
      <Redirect
        ...
        to={{
          pathname: "/osprint",
          state: {
            selecionados: props.dadosOriginais.filter(item => props.selecionados.find(selecionado => selecionado.numeroOrdemServico === item['número_da_ordem_de_serviço'] ))
        }
        }}
      >
      </Redirect>
...

      {
        props.selecionados.length > 0 ? (
          <Tooltip title="Gerar Ordem Serviço">
            <Button onClick={(event) => abrirImpressao(event)} variant="outlined">Gerar arquivo de impressão</Button>
          </Tooltip>
        ) : (
